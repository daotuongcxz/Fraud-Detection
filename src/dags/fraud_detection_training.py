...
class FraudDetectionTraining:
    ...
    def read_from_kafka(self) -> pd.DataFrame:
        ...
        try:
            ...
            security_protocol = kafka_conf.get('security_protocol', 'PLAINTEXT')
            consumer_timeout = kafka_conf.get('timeout', 10000) 

            print("Kafka config - topic: %s, servers: %s, protocol: %s", topic, bootstrap_servers, security_protocol)
    ...
    def read_from_database(self) -> pd.DataFrame:
        """
        Secure PostgreSQL data loader with production-grade features:
        ...
        Returns:
            pd.DataFrame: Transaction data matching Kafka schema
        """
        try:
            import psycopg2
            ...
            # Database configuration - consider moving to config.yaml
            db_config = {
            ...
            # Connect with connection pooling considerations
            conn = psycopg2.connect(
                **db_config,
                cursor_factory=DictCursor,
                application_name="fraud_detection_training"
            )
            
            # Batch query parameters
            batch_size = self.config.get('database', {}).get('batch_size', 10000)
            query = sql.SQL("""
                SELECT 
                    transaction_id,
                    user_id,
            ...
            # Get most recent data (or implement incremental loading)
            ...
            if not rows:
                logger.warning('No new transactions found since %s', last_timestamp)
                return pd.DataFrame()
                
            # Convert to DataFrame with same schema as Kafka version
            ...
            # Data quality checks
            if df.empty:
                raise ValueError('Empty DataFrame returned from database')
                
            if 'is_fraud' not in df.columns:
                raise ValueError('Fraud label (is_fraud) missing from database data')
            ...
            fraud_rate = df['is_fraud'].mean() * 100
            logger.info('Database data loaded successfully with fraud rate: %.2f%%', fraud_rate)
            
            return df
            
        except Exception as e:
            logger.error('Failed to read data from database: %s', str(e), exc_info=True)
            raise
        finally:
            if 'conn' in locals():
                conn.close()
